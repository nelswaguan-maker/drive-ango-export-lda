...
begin
  ...
  if tg_op='INSERT' then if not public.admin_has_permission('publish') then raise exception 'Sem permissão para publicar anúncios'; end if; return new; end if;
...
create policy "Admins can insert drive cars" on public.drive_cars for insert to authenticated with check (public.is_current_user_admin());
drop policy if exists "Admins can update drive cars" on public.drive_cars;
create policy "Admins can update drive cars" on public.drive_cars for update to authenticated using (public.is_current_user_admin()) with check (public.is_current_user_admin());
